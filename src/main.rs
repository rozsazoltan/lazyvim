...
fn ensure_portable_lazyvim_config(config_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    remove_unsupported_portable_treesitter_config(config_dir)
}
...
fn remove_unsupported_portable_treesitter_config(
    ...
    let content = fs::read_to_string(&portable_plugin)?;
    let generated_by_launcher = content.contains("Generated by the portable LazyVim launcher");
    let sets_unsupported_compilers = content.contains("install.compilers")
        || content.contains("opts.install.compilers")
        || content.contains("compilers =");

    if generated_by_launcher || sets_unsupported_compilers {
        fs::remove_file(portable_plugin)?;
    }
